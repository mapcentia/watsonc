  ...
  switch (action.type) {
    ...
    case "ADD_BOREHOLE_FEATURE":
      ...
      for (let i = 0; i < features.length; i++) {
        if (
          features[i].properties.relation ===
            action.payload.properties?.relation &&
          features[i].properties.loc_id === action.payload.properties?.loc_id
        ) {
          found = true;
          break;
        }
    ...
    case "SELECT_MEASUREMENT_COUNT":
      return Object.assign({}, state, {
        selectedMeasurementCount: action.payload,
      });
    case "SELECT_LAYER":
      compoundLayerKey =
        action.payload.originalLayerKey +
        (action.payload.additionalKey
          ? `#${action.payload.additionalKey}`
          : ``);
